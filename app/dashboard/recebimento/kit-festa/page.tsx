...
import { ArrowLeftIcon, PrinterIcon, DownloadIcon, Trash2, BarChart2, AlertTriangle } from 'lucide-react'
...
import { KitFestaVehicleCardList } from '@/components/kit-festa-vehicle-card-list'
import { KitFestaOccurrenceList } from '@/components/kit-festa-occurrence-list'
import { KitFestaBarChart } from '@/components/kit-festa-bar-chart'
...
export default function KitFestaPage() {
  ...
  useEffect(() => {
    ...
    setOccurrences([...loadedOccurrences, ...loadedArchivedOccurrences]);
  ...
  const kitFestaVehicles = useMemo(() => {
    return vehicles.filter(vehicle =>
      normalizeString(vehicle.tipo || '') === normalizeString("Kit Festa")
    );
  }, [vehicles]);

  const kitFestaOccurrences = useMemo(() => {
    const kitFestaPlacas = new Set(kitFestaVehicles.map(v => v.placa));
    return occurrences.filter(occ => kitFestaPlacas.has(occ.placa));
  }, [kitFestaVehicles, occurrences]);
  ...
  const handleDownloadPdf = () => {
    if (kitFestaVehicles.length === 0) {
      alert("Não há veículos Kit Festa para baixar.");
      return;
    }
    ...
    doc.setFontSize(18);
    doc.text('Veículos Kit Festa Registrados', 14, yOffset);
    yOffset += 10;
    doc.setFontSize(12);
    doc.text(`Data de Geração: ${format(new Date(), 'dd/MM/yyyy HH:mm:ss', { locale: ptBR })}`, 14, yOffset);
    yOffset += 15;

    const headers = ["Doca", "Carga", "Placa", "Temperatura", "Lacre", "Status", "Ocorrência", "Perfil", "Chegada", "Agendamento", "Contêiner", "Motorista", "Transportadora"]; // Adicionado Contêiner
    const data = kitFestaVehicles.map(vehicle => {
    ...
    doc.save(`kit_festa_veiculos_${format(new Date(), 'ddMMyyyy_HHmmss')}.pdf`);
  };

  const handlePrint = () => {
    if (kitFestaVehicles.length === 0) {
      alert("Não há veículos Kit Festa para imprimir.");
      return;
    }
    ...
    <div className="flex flex-col gap-6 p-4 md:p-6 max-w-6xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 gap-4">
        ...
        <div className="flex items-center justify-center sm:justify-end gap-4 flex-grow">
          ...
          />
          <h1 className="text-2xl font-bold text-center">Informações de Kit Festa</h1>
        </div>
      </div>
      <p className="text-center text-muted-foreground">Detalhes e histórico de veículos e ocorrências relacionados a Kit Festa.</p>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1">
          <KitFestaBarChart count={kitFestaVehicles.length} />
        </div>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                ...
                Ocorrências de Kit Festa
              </CardTitle>
            </CardHeader>
            <CardContent>
              <KitFestaOccurrenceList occurrences={kitFestaOccurrences} />
            </CardContent>
          </Card>
        </div>
      </div>

      <h2 className="text-xl font-semibold mb-4 text-center mt-6">Veículos Kit Festa</h2>
      ...
      {isMobile ? (
        <KitFestaVehicleCardList vehicles={kitFestaVehicles} onRemoveVehicle={handleRemoveVehicle} drivers={drivers} userRole={userRole} />
      ) : (
        <div className="w-full overflow-x-auto border rounded-lg shadow-sm">
          <Table>
            <TableCaption>Detalhes dos veículos do tipo Kit Festa.</TableCaption>
            <TableHeader>
              <TableRow>
                <TableHead className="w-[80px]">Doca</TableHead><TableHead>Carga</TableHead><TableHead>Placa</TableHead><TableHead>Temperatura</TableHead><TableHead>Lacre</TableHead><TableHead>Status</TableHead><TableHead>Ocorrência</TableHead><TableHead>Perfil</TableHead><TableHead>Contêiner</TableHead><TableHead>Chegada</TableHead><TableHead>Agendamento</TableHead><TableHead>Motorista</TableHead><TableHead>Transportadora</TableHead><TableHead className="text-right">Ações</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {kitFestaVehicles.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={14} className="text-center py-8 text-muted-foreground">
                    Nenhum veículo encontrado do tipo "Kit Festa".
                  </TableCell>
                </TableRow>
              ) : (
                kitFestaVehicles.map((vehicle) => {
                  const driver = getDriverInfo(vehicle.driverId);
                  return (
                    <TableRow
                      key={vehicle.id}
                    ...
                    >
                      ...
                      <TableCell>{vehicle.containerNumber || 'N/A'}</TableCell> {/* Adicionado */}
