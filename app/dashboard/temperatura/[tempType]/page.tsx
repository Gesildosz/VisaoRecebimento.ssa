...
export default function VehicleTemperatureListPage() {
  const router = useRouter()
  const params = useParams()
  const tempType = (params.tempType as string | undefined) || '';
  ...
  useEffect(() => {
    const filtered = vehicles.filter(vehicle =>
      normalizeString(vehicle.temperatura || '') === normalizeString(tempType)
    );
    setFilteredVehicles(filtered);
  }, [vehicles, tempType]);

  const getTitleForTemperature = (type: string) => {
    switch (normalizeString(type)) {
      case 'congelado': return 'Veículos Congelados';
      case 'resfriado': return 'Veículos Resfriados';
      case 'mista': return 'Veículos com Temperatura Mista';
      case 'seca': return 'Veículos com Temperatura Seca';
      default: return `Veículos com Temperatura: ${type}`;
    }
  };
  ...
  const handleDownloadPdf = () => {
    if (filteredVehicles.length === 0) {
      alert("Não há veículos para baixar neste tipo de temperatura.");
      return;
    }
    ...
    doc.setFontSize(18);
    doc.text(getTitleForTemperature(tempType), 14, yOffset);
    yOffset += 10;
    doc.setFontSize(12);
    ...
    doc.save(`${tempType}_vehicles_${format(new Date(), 'ddMMyyyy_HHmmss')}.pdf`);
  };

  const handlePrint = () => {
    if (filteredVehicles.length === 0) {
      alert("Não há veículos para imprimir neste tipo de temperatura.");
      return;
    }
    ...
    <div className="flex flex-col gap-6 p-4 md:p-6 max-w-5xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 gap-4">
        ...
        <div className="flex items-center justify-center sm:justify-end gap-4 flex-grow">
          ...
          />
          <h1 className="text-2xl font-bold text-center">{getTitleForTemperature(tempType)}</h1>
        </div>
      </div>
      <p className="text-center text-muted-foreground">Lista de veículos com temperatura: {tempType}.</p>
      ...
      {isMobile ? (
        ...
        <div className="w-full overflow-x-auto border rounded-lg shadow-sm">
          <Table>
            <TableCaption>Detalhes dos veículos com temperatura {tempType}.</TableCaption>
            <TableHeader>
              <TableRow>
              ...
              {filteredVehicles.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={13} className="text-center py-8 text-muted-foreground">
                    Nenhum veículo encontrado com temperatura "{tempType}".
                  </TableCell>
                </TableRow>
              ) : (
