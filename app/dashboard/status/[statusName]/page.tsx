...
export default function VehicleStatusListPage() {
  ...
  const statusName = (params.statusName as string | undefined) || ''; 
  ...
  useEffect(() => {
    const filtered = vehicles.filter(vehicle =>
      normalizeString(vehicle.status) === normalizeString(statusName)
    );
    setFilteredVehicles(filtered);
  }, [vehicles, statusName]);

  const getTitleForStatus = (status: string) => {
    switch (normalizeString(status)) {
      case 'em-descarga': return 'Veículos Em Descarga';
      case 'patio': return 'Veículos no Pátio';
      case 'finalizado': return 'Veículos Finalizados';
      default: return `Veículos com Status: ${status}`;
    }
  };
  ...
  const handleDownloadPdf = () => {
    if (filteredVehicles.length === 0) {
      alert("Não há veículos para baixar neste status.");
      return;
    }
    ...
    doc.setFontSize(18);
    doc.text(getTitleForStatus(statusName), 14, yOffset);
    yOffset += 10;
    doc.setFontSize(12);
    ...
    doc.save(`${statusName}_vehicles_${format(new Date(), 'ddMMyyyy_HHmmss')}.pdf`);
  };

  const handlePrint = () => {
    if (filteredVehicles.length === 0) {
      alert("Não há veículos para imprimir neste status.");
      return;
    }
    ...
    <div className="flex flex-col gap-6 p-4 md:p-6 max-w-5xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 gap-4">
        ...
        <div className="flex items-center justify-center sm:justify-end gap-4 flex-grow">
          ...
          />
          <h1 className="text-2xl font-bold text-center">{getTitleForStatus(statusName)}</h1>
        </div>
      </div>
      <p className="text-center text-muted-foreground">Lista de veículos com status: {statusName}.</p>
      ...
      {isMobile ? (
        ...
        <div className="w-full overflow-x-auto border rounded-lg shadow-sm">
          <Table>
            <TableCaption>Detalhes dos veículos com status {statusName}.</TableCaption>
            <TableHeader>
              <TableRow>
              ...
              {filteredVehicles.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={13} className="text-center py-8 text-muted-foreground">
                    Nenhum veículo encontrado com status "{statusName}".
                  </TableCell>
                </TableRow>
              ) : (
